package nodejs.interop;
//package net.plan99.nodejs.java;

//import nodejs.helpers.NodeJSAPI;
...
public class NodeJS {
    ...
    @Deprecated
    public static void boot(LinkedBlockingDeque<Runnable> taskQueue, Value evalFunction, String[] args)
    ...
    private static void boot1(LinkedBlockingDeque<Runnable> taskQueue, Value evalFunction, String[] args)
        ...
        if (args.length == 0) {
            ...
            // Use the parent classloader to forcibly toss out this version of the interop
            // JAR, to avoid confusion
            // later when there are two classloaders in play, BUT, holepunch this specific
            // class and inner classes
            // through so the state linked up from the bootstrap script is still here. In
            // other words, this class is
    ...
    /**
     * Returns true if executing within the main NodeJS thread. Note: does NOT
     * return true if you are executing on some other worker you created yourself.
     */
    ...
    public static void checkOnMainNodeThread() {
        if (!isOnMainNodeThread())
            throw new IllegalStateException(
                    "You are not currently on the NodeJS thread and thus cannot access the JavaScript world. Surround your calls to JS with NodeJS.runJS(), NodeJS.runJSAsync() or the Kotlin nodejs{} block.");
    }

    /**
     * This {@link Executor} runs the given commands on the NodeJS event loop
     * thread. The other utility functions on this class are all just utilities that
     * delegate to this executor. If you are on the nodejs thread already, its run
     * immediately.
     */
    ...
    /**
     * Schedules execution of the given {@link Supplier} onto the NodeJS thread, and
     * returns a future that will complete when it's been run.
     */
    ...
    /**
     * Runs the given {@link Supplier} on the NodeJS thread, blocks until execution
     * has been performed and returns the result that was computed.
     */
    ...
    /**
     * Evaluates the given string and returns a generic {@link Value} object
     * representing the result, that can then be converted into more useful forms.
     * Note that you must be on the NodeJS thread for this to work (see
    ...
    /**
     * Converts the {@link Value} to a JVM type in the following way:
     * <p>
     *
     * <ol>
     * <li>If the type is an interface not annotated with
     * {@code @FunctionalInterface} then a special proxy is returned that knows how
     * to map JavaBean style property methods on that interface to JavaScript
     * properties.</li>
     * <li>Otherwise, the {@link Value#as} method is used with the
     * {@link TypeLiteral} so generics are preserved and the best possible
     * translation occurs.</li>
     * </ol>
     */
